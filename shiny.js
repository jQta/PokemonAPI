...
const printPokemon = (pokemons) => {
    ...
    for (let pokemon of pokemons) {
        ...
        if (filterName || filterMainType || filterSecondType) {
            ...
            description$$.addEventListener("click", () => { chosenPokemon(pokemon) });
        }
    }
};
...
const chosenPokemon = (pokemon) => {
    ...
    image$$.addEventListener("click", () => { flipItOut(pokemon) });
    description$$.addEventListener("click", restart);

    profile$$.appendChild(button$$);
    button$$.innerHTML = "Volver atrás";
